...
/// @notice Deploys Optimism Sepolia dummy assets, grants approvals, and registers them with the OP burn collector.
contract DeployOpDummies is Script {
    function run() external {
        uint256 deployerKey = vm.envUint("PRIVATE_KEY");
        address collectorAddr = vm.envAddress("OP_BURN_COLLECTOR_ADDRESS");
        ...
        console.log("DummyOneOfOne (OP) deployed:", address(dummy721));
        console.log("DummyEdition1155 (OP) deployed:", address(dummy1155));
        console.log("DummyERC20Burnable (OP) deployed:", address(dummy20));
        ...
        console.log("  export OP_DUMMY_ONE_OF_ONE_ADDRESS=%s", address(dummy721));
        console.log("  export OP_DUMMY_EDITION1155_ADDRESS=%s", address(dummy1155));
        console.log("  export OP_DUMMY_ERC20_ADDRESS=%s", address(dummy20));
    }
}
