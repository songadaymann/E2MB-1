...
/**
 * @title Step 3: Add Circle and Rotating Ball
 * @notice Add circle border and ball that rotates around it
 */
contract Step3_CircleAndBall is Script {
    ...
    function run() external {
        ...
        string memory html = string(abi.encodePacked(
            ...
            '</div></div></div>',
            '<div id="debug">',
            '<div>Blocks: <span id="currentBlocks"></span> | Elapsed: <span id="elapsed"></span>s | Ball angle: <span id="ballAngle"></span>&deg;</div>',
            '</div>',
            '<script>',
            _digitPaths(),
            ...
            'document.getElementById("ballAngle").textContent=angleDeg.toFixed(1);',
            '}',
        ...
        vm.writeFile("OUTPUTS/html-countdown-step3.html", html);
        console.log("Generated: OUTPUTS/html-countdown-step3.html");
        console.log("Should show circle with ball rotating around edge");
    }
